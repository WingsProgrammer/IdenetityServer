...
public static partial class ServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddCustomIdentity(
        ...
        Action<IdentityOptions>? configure = null)
    {
        // Problem with .net core identity - will override our default authentication scheme `JwtBearerDefaults.AuthenticationScheme` to unwanted `MS.Services.Identity.Application` in `AddIdentity()` method .net identity
        ...
        if (configuration.GetValue<bool>("PostgresOptions.UseInMemory"))
        {
            services.AddDbContext<IdentityContext>(options =>
                options.UseInMemoryDatabase("Shop.Services.MS.Services.Identity"));
